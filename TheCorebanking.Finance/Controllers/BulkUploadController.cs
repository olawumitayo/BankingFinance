...
namespace TheCorebanking.Finance.Controllers
{
   // [Authorize]
    ...
    {
        private ILogger<BulkUploadController> logger;
        ...
        public BulkUploadController(ISetupUnitOfWork uowSetup, IHostingEnvironment hostingEnvironment, ILogger<BulkUploadController> financeLogger)
       ...
       //[AllowAnonymous]
        ...
        public JsonResult Index(decimal availablebalance, TblSingleFundTransfer transaction)
        {
            ...
            ///(string AccountDr, string AccountCr, decimal Amount, int OperationType, int TransactionType, int OperationID, string CreateBy, string NarrationDr, string NarrationCr, string PostingTime, string Reference, string TransCode, string BrCode, string CoyCode, bool InEntryState, DateTime PostDate, DateTime ValueDate, bool IsCheque, int Appsource, string ChequeNo, bool ChargeStamp, int ChargeType)
            TransID = setupUnitOfWork.Transaction.InsertFund(transaction.AccountDr, transaction.AccountCr, transaction.Amount, ddlOperationType, Convert.ToInt32(transaction.TransactionType), Convert.ToInt32(PostType.SINGLETRANSFER), logUser, transaction.NarrationDr, transaction.NarrationCr, DateTime.Now.ToShortTimeString(), transSequence, transCode, branch, getbranch.CompanyId, EntryState, Convert.ToDateTime(transaction.PostDate), DateTime.Now, isCheque, 0, transaction.ChequeNo, transaction.ChargeStamp, 0);
       ...
       [AllowAnonymous]
        ...
        [HttpPost]
        public JsonResult MultipleTransfer(List<multipleTransfer> transfers, TotalResult totalResult, string BatchName, bool IsJournal, bool IsReciept)
        {
            ...
            if (totalResult.TotalCredit == totalResult.TotalDebit)
            {
                ...
                var Ref = "Batch" + "/" + rand.Next().ToString().Substring(0, 7);
                int success = 0;
                ...
                foreach (var Multiple in transfers)
                {
                    ...
                    if (string.IsNullOrEmpty(listMultiple.ToString())==true)
                    {
                        return Json(new { message = " " });
                    }
                    ...
                    var existMultiple = setupUnitOfWork.MultipleFund.GetAll();
                    //if (existMultiple.Count() > 0)
                    //{
                    //    var multipleReference = setupUnitOfWork.MultipleFund.GetAll().FirstOrDefault();
                    //    if (string.IsNullOrEmpty(multipleReference.ToString()) ==true)
                    //    {
                    //        setupUnitOfWork.Transaction.InsertMultipleFund(existMultiple.FirstOrDefault().Reference, listMultiple.AccountNo, listMultiple.BrCode, listMultiple.CoyCode, Convert.ToDecimal(listMultiple.Amount), Convert.ToInt32(listMultiple.OperationId), listMultiple.Narration, listMultiple.TransCode, Convert.ToDateTime(listMultiple.ValueDate), Convert.ToDateTime(listMultiple.DateCreated), logUser, listMultiple.InEntryState, listMultiple.TransactionType, listMultiple.OperationType, BatchName, listMultiple.AppSource, IsJournal, IsReciept, false, false);
                    //    }
                    //    setupUnitOfWork.Transaction.InsertMultipleFund(multipleReference.Reference, listMultiple.AccountNo, listMultiple.BrCode, listMultiple.CoyCode, Convert.ToDecimal(listMultiple.Amount), Convert.ToInt32(listMultiple.OperationId), listMultiple.Narration, listMultiple.TransCode, Convert.ToDateTime(listMultiple.ValueDate), Convert.ToDateTime(listMultiple.DateCreated), logUser, listMultiple.InEntryState, listMultiple.TransactionType, listMultiple.OperationType, BatchName, listMultiple.AppSource, IsJournal, IsReciept, false, false);


                    //}

                        var resultSP = setupUnitOfWork.Transaction.InsertMultipleFund(Ref, listMultiple.AccountNo, listMultiple.BrCode, listMultiple.CoyCode, Convert.ToDecimal(listMultiple.Amount), Convert.ToInt32(listMultiple.OperationId), listMultiple.Narration, listMultiple.TransCode, Convert.ToDateTime(listMultiple.ValueDate), Convert.ToDateTime(listMultiple.DateCreated), logUser, listMultiple.InEntryState, listMultiple.TransactionType, listMultiple.OperationType, BatchName, listMultiple.AppSource, IsJournal, IsReciept, false, false,totalResult.ChargeStamp);
                ...
                //tblBankingAudit.TransType = " Save Transaction With Batchref - " + ;
                //tblBankingAudit.TransTime = TimeStamp.ToShortTimeString();
                //tblBankingAudit.Status = "Successful";
                //tblBankingAudit.CmpName = "FINTRAK";
                //setupUnitOfWork.Audit.Add(tblBankingAudit);
                //setupUnitOfWork.Commit();
                return Json(new { message = " " });
        ...
        public JsonResult TempTransfer(transactionModel transaction)
        {
            ...
            {
                return Json(new { message = "You do not have the previledge to perform these transaction. Total amount is greater than your cummulative amount limit!" });
            }
        ...
        public async Task<List<TblBankingBatchTransferUploadTemp>> Import(IFormFile formFile,UploadTempplate upload, CancellationToken cancellationToken)
        {
            ...
            var BatchRef = "TRU/" + branch + "/" + rand.Next().ToString().Substring(0, 7);
            var list = new List<TblBankingBatchTransferUploadTemp>();
            ...
            using (var stream = new MemoryStream())
            {
               await formFile.CopyToAsync(stream);
                ...
                using (var package = new ExcelPackage(stream))
                {
                    ...
                    {
                        var customer = worksheet.Cells[row, 1].Value.ToString();
                        var account = CustomerAndGL().Where(o => o.Accountnumber == customer).FirstOrDefault().AccountName;
                        var balance = CustomerAndGL().Where(o => o.Accountnumber == customer).FirstOrDefault().AVAILABLEBALANCE;
                        string date = worksheet.Cells[row, 1].Value.ToString();
                        int first = date.IndexOf('-');
                        int last = date.LastIndexOf('-');
                        list.Add(new TblBankingBatchTransferUploadTemp
                        {
                            ...
                            IsCheque = Convert.ToBoolean(worksheet.Cells[row, 2].Value),
                            ChequeNo = Convert.ToString(worksheet.Cells[row, 3].Value),
                            Amount = Convert.ToDecimal(worksheet.Cells[row, 4].Value),
                            Narration = worksheet.Cells[row, 5].Value.ToString(),
                            StampDuty = Convert.ToBoolean(worksheet.Cells[row, 6].Value),
                            AccountName = account,
                ...
                setupUnitOfWork.BatchUpload.AddRange(list);
                setupUnitOfWork.Commit();
            }
        ...
        public JsonResult loadBatchRef()
        {
            ...
            var result = (from multiple in setupUnitOfWork.MultipleFund.GetAll().Where(o => o.Approved == true)
                          group multiple by multiple.Reference into ReferenceMultiple
                          select ReferenceMultiple.OrderByDescending(o => o.Id).First()
                          );
      ...
      [HttpPost]
        ...
        [HttpPost]
        public JsonResult AddTemplate(UploadTempplate upload)
        {
