...
namespace TheCoreBanking.Finance.Data.Repository
{
    ...
    {
        public TransactionRepository(TheCoreBankingContext context) : base(context) { }
        ...
        public decimal SPGLBalance(string accountNo)
        {
            ...
            using (var context = new TheCoreBankingContext())
            {
                ...
                GLBalance = context.GLBalance.FromSqlRaw("General.GetGLBalance2 @AccountID", AcctNo).AsEnumerable().FirstOrDefault();
        ...
        public decimal CustomerBalance(string accountNo)
        {
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    ...
                    //decimal? Balance = 0;
                    GetCustomerBalance = context.GetCustomerBalance.FromSqlRaw("dbo.GetCustomerBalance @AccountNumber", AcctNo).AsEnumerable().FirstOrDefault();
        ...
        public List<sp_MultipleandSingle> spMultipleSingle(string TranDate, string initiators)
        {
            ...
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    ...
                    return context.sp_MultipleandSingle.FromSqlRaw("dbo.sp_MultipleandSingle @date,@initiator", date, initiator).AsEnumerable().ToList();
        ...
        public List<sp_ListBulkReversal> spBulkList(string TranDate, string initiators)
        {
            List<sp_ListBulkReversal> multipleSingle = new List<sp_ListBulkReversal>();
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    SqlParameter date = new SqlParameter("@date", TranDate);
                    SqlParameter initiator = new SqlParameter("@initiator", initiators);
                    multipleSingle = context.sp_ListBulkReversal.FromSqlRaw("dbo.sp_CustomerReversalList @date,@initiator", date, initiator).AsEnumerable().ToList();
                    //return context.sp_ListBulkReversal.FromSqlRaw("dbo.sp_Batchupload @date,@initiator", date, initiator).ToList();
                    return multipleSingle;
                }
            }
            catch (Exception ex)
        ...
        public List<sp_ListGL> spGLList(string TranDate, string initiators)
        {
            List<sp_ListGL> multipleSingle = new List<sp_ListGL>();
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    SqlParameter date = new SqlParameter("@date", TranDate);
                    SqlParameter initiator = new SqlParameter("@initiator", initiators);
                    multipleSingle = context.sp_ListGL.FromSqlRaw("dbo.sp_GLReversalList @date,@initiator", date, initiator).AsEnumerable().ToList();
                    //return context.sp_ListBulkReversal.FromSqlRaw("dbo.sp_Batchupload @date,@initiator", date, initiator).ToList();
                    return multipleSingle;
                }
            }
        ...
        public List<sp_MultipleandSingle> spMultipleSingleGL(string TranDate, string initiators)
        {
            List<sp_MultipleandSingle> multipleSingle = new List<sp_MultipleandSingle>();
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    SqlParameter date = new SqlParameter("@date", TranDate);
                    SqlParameter initiator = new SqlParameter("@initiator", initiators);
                    return context.sp_MultipleandSingle.FromSqlRaw("dbo.sp_MultipleandSingleGL @date,@initiator", date, initiator).AsEnumerable().ToList();
                    //return multipleSingle;
                }
            }
            catch (Exception ex)
        ...
        public List<sp_Single> spSingle(string TranDate, string initiators)
        {
            ...
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    SqlParameter date = new SqlParameter("@date", TranDate);
                    SqlParameter initiator = new SqlParameter("@initiator", initiators);
                    return context.sp_Single.FromSqlRaw("dbo.sp_Single @date,@initiator", date, initiator).AsEnumerable().ToList();
                    //return multipleSingle;
                }
            }
            catch (Exception ex)
        ...
        public List<sp_Single> spSingleGL(string TranDate, string initiators)
        {
            List<sp_Single> Single = new List<sp_Single>();
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    SqlParameter date = new SqlParameter("@date", TranDate);
                    SqlParameter initiator = new SqlParameter("@initiator", initiators);
                    return context.sp_Single.FromSqlRaw("dbo.sp_SingleGL @date,@initiator", date, initiator).AsEnumerable().ToList();
                    //return multipleSingle;
                }
            }
            catch (Exception ex)
        ...
        public int InsertFund(string AccountDr, string AccountCr, decimal Amount, int OperationType, int TransactionType, int OperationID, string logUser, string NarrationDr, string NarrationCr, string PostingTime, string Reference, string TransCode, string BrCode, string CoyCode, bool InEntryState, DateTime PostDate, DateTime ValueDate, bool IsCheque, int Appsource, string ChequeNo, bool ChargeStamp, int ChargeType)
        {
            ...
            using (var context = new TheCoreBankingContext())
            {
                ...
                return context.Database.ExecuteSqlRaw("InsertFundTransfer @AccountDr,@AccountCr, @Amount,@OperationType,@TransactionType,@OperationID,@CreateBy,@NarrationDr,@NarrationCr,@PostingTime,@Reference,@TransCode,@BrCode,@CoyCode,@InEntryState,@PostDate,@ValueDate,@IsCheque,@Appsource,@ChequeNo,@ChargeStamp,@ChargeType", _AccountDr, _AccountCr, _Amount, _OperationType, _TransactionType,  _OperationID, _CreateBy, _DrNarration, _CrNarration, _PostingTime, _Reference, _TransCode, _BrCode, _CoyCode, _InEntryState, _PostDate, _ValueDate, _IsCheque, _Appsource, _ChequeNo, _ChargeStamp, _ChargeType);
        ...
        public List<GetAccountType> ChartOfAccountNumber(string AccounttypeID)
        {
            ...
            try
            {
                ...
                using (var context = new TheCoreBankingContext())
                {
                    ...
                    return context.GetAccountType.FromSqlRaw("sp_BankingChartOfAccountNumber @AccounttypeID", _AccounttypeID).AsEnumerable().ToList();
